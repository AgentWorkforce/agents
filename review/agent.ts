...
export function reviewHarnessPrompt(pr: { owner: string; repo: string; number: number }): string {
  return [
    ...
    `Stay within this PR's purpose (.workforce/pr.diff is the change; .workforce/context.json carries the PR title`,
    `and description — what it is actually about). A reviewer suggestion that changes files or behavior unrelated to`,
    ...
    `cleanup — does NOT belong in this PR: record it as an advisory note in your review and leave the code unchanged.`,
    ...
    `find what CI actually runs; install dependencies if needed) so you catch breakage DOWNSTREAM of the file you`,
